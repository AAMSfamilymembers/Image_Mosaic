...
def stitch(image3,m):
	global canvas
	dst = cv2.warpPerspective(image3,m,(canvas.shape[1],canvas.shape[0]))
	#ind1 = np.where(canvas==0)
	ind_common = np.where((canvas!=0) & (dst!=0))
	canvas[ind_common] = canvas[ind_common]*0.8 + dst[ind_common]*0.2
...
canvas = cv2.warpPerspective(img1,h,(canvas1.shape[1],canvas1.shape[0]),flags=cv2.INTER_LINEAR)
n=0
cap = cv2.VideoCapture(0)
cv2.waitKey(3000)
_,frame = cap.read()
