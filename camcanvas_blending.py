...
def stitch(image3,m):
	...
	error = np.mean(np.subtract(canvas[ind_common],dst[ind_common]),dtype = int)	
	#canvas[ind_common] = canvas[ind_common]* 0.1 + dst[ind_common]*0.9
	ind1 = np.where((canvas==0) & (dst!=0))	
	canvas[ind1] = (dst[ind1] + error).astype(np.uint8)		
...
_,frame = cap.read()
img2 = frame
cv2.imwrite('image' + str(n+1) + '.jpg',img2)
cap.release()
while(n<3):
...
cv2.imwrite('canvas_blending.jpg',canvas)
